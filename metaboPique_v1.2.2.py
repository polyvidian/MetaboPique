...
def smp_theCompositer(subrawData, compositeQu):
	...
	for toComposite in subrawData:
		...
		if finalMajoritySpec is not False:
			...
			compositeQu.put([groupFolderName.split("/")[-1], predictedAdduct, PEPMASS, specCount, XICoutput, finalMajoritySpec])
...
while len(sortedSpectrumData) > 0:
	...
	for aGroup in theBatch:
		...
		for keys in spectrumKeys:
			...
			if (gaussBool | takeItAll):
				...
				avgMZ += _mz  # this was moved, only collects m/z values from "valid" specrra now

				lastValidSpec = [chromKey, (_mz, _rt), _precScanNo, _precInt]

		if (gaussBool | takeItAll):
			specCount += len(spectrumKeys)
			lastValidChromStatus = gaussBool  # really only used for singleton spectra to output gaussianness
	...
	# ---------composite spectrum code
	if specCount > 1:
		textInfo = [min(allavgMZs), predictedAdduct, specCount, takeItAll]
		...
		PEPMASS = round(avgMZ, 4)
		mgfWriter.write("BEGIN IONS\nTITLE=%s\nCHARGE=%s\nPEPMASS=%s\nCOMMENT=Type:SingletonSpectrum; SourceChromatogram:%s; Scan:%s; PrecIntensity:%s; XIC:%s\n" % (groupFolderName.split("/")[-1], predictedAdduct,
																																										PEPMASS, chromKey, scanNo, precInt, XICoutput))
		for peak in theSpectrum:
			_peakMZtext = str(round(peak[0], 4))
			_peakAbtext = str(peak[1])
			mgfWriter.write("%s %s\n" % (_peakMZtext, _peakAbtext))
